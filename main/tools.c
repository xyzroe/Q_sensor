...
const char *get_endpoint_name(int endpoint)
{
    ...
    case AHT20_ENDPOINT:
        return "AHT20";
    case BMP280_ENDPOINT:
        return "BMP280";
    case SCD40_ENDPOINT:
        return "SCD40";
    case BH1750_ENDPOINT:
        return "BH1750";
    case AGS10_ENDPOINT:
        return "AGS10";
    case IMU_BASE_ENDPOINT:
        return "IMU_FIRST";
    default:
        return "Unknown";
    }
...
void set_bit(uint8_t *byte, uint8_t bit_index, bool value)
{
    if (bit_index >= 8)
    {
        ESP_LOGE(__func__, "Invalid bit index: %d", bit_index);
        return;
    }
...
void heap_stats()
{
    ...
    ESP_LOGE(__func__, "free: %.2f%% (%d bytes), fragmentation: %.2f%%",
